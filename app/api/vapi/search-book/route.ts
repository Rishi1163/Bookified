...
async function processBookSearch(bookId:unknown, query: unknown) {
    ...
    const segments = searchResult.data as SearchSegment[]
...
export async function POST(request: Request) {
    try {
        ...
        const toolCallList = body?.message?.toolCallList || body?.message?.toolCallList
        ...
        for (const toolCall of toolCallList) {
            ...
            if(name === 'searchbook') {
                ...
                results.push({toolCallId: id, result: `Unknown funciton ${name}`})
            }
